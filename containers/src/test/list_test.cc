...
class ListTest : public ::testing::Test {
 protected:
  ...
  vv::list<testClass> ll = vv::list<testClass>();
  std::list<testClass> stdll_il = {{"first", "second"},
                                   {"firstfirst", "secondsecond"}};
  vv::list<testClass> ll_il = {{"first", "second"},
                               {"firstfirst", "secondsecond"}};

  const std::list<testClass> const_std_list = {{"a", "b"}, {"c", "d"}};
  const vv::list<testClass> const_list = {{"a", "b"}, {"c", "d"}};

  template <typename T>
  static void check_with_std(const vv::list<T>& ll, const std::list<T>& stdll) {
...
TEST_F(ListTest, swap) {
  vv::list<testClass> list_for_swap = {{"1", "2"}, {"31", "23"}, {"12", "22"}};
  std::list<testClass> stdlist_for_swap = {
      {"1", "2"}, {"31", "23"}, {"12", "22"}};

  vv::list<testClass> list_for_swap2 = {{"dasd", "sad"}, {"gdf", "asda"}};
  std::list<testClass> stdlist_for_swap2 = {{"dasd", "sad"}, {"gdf", "asda"}};

  vv::list<testClass> list_for_swap3;      // empty
...
TEST_F(ListTest, EntireListModifiers) {
  vv::list<testClass> non_empty = {{"dasd", "sad"}, {"dasd", "sad"},
                                   {"gdf", "asda"}, {"dsd", "2323"},
                                   {"dasd", "sad"}, {"1", "2"}};
  std::list<testClass> std_non_empty = {{"dasd", "sad"}, {"dasd", "sad"},
                                        {"gdf", "asda"}, {"dsd", "2323"},
                                        {"dasd", "sad"}, {"1", "2"}};
  vv::list<testClass> for_splice = {{"8", "6"}, {"4", "5"}, {"1", "2"},
                                    {"0", "0"}, {"0", "0"}, {"0", "0"}};
...
TEST_F(ListTest, Ordering) {
  vv::list<int> int_list = {1, 3, 3, 5, 7, 11, 1337};
  std::list<int> std_int_list = {1, 3, 3, 5, 7, 11, 1337};
  vv::list<int> int_list2 = {2, 4, 8, 16, 32, 32, 64, 128, 1024};
  ...
  vv::list<int> int_list3 = {32,  23,   24, 214, 23,  4523, 512,
...
TEST_F(ListTest, InsertMany) {
  vv::list<testClass> test_list = {{"1", "2"}};
